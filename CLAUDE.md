...
- **tmux** — Prefix is `Ctrl+Space`. Plugins managed by TPM (plugins dir is gitignored). Catppuccin theme. vim-tmux-navigator for seamless vim/tmux pane switching. `live-sessions.sh` records which sessions are open so a reboot does not take the list of in-progress work with it; `session-created`/`session-closed` hooks maintain it, and a server killed by signal fires neither, which is exactly why the file outlives the reboot. `prefix f` then lists the recorded-but-not-running sessions dimmed alongside the live ones and rebuilds one on selection. `claude-continue.sh` opens the session's claude window on the conversation the directory already holds, or on a fresh one where it holds none: `claude -c` errors out instead of falling back, which left every new session staring at a bare shell. Tests: `tmux/.config/tmux/tests/`.
